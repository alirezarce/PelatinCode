...
import VideOfflineCourseSchema from '../../schemas/videoOfflineCourse.js';

import {log,toObjectId,getEnv} from '../../core/utils.js';
import datetime from '../../core/datetime.js';
import crypto from '../../core/crypto.js';
import CategoryOfflineCourseSchema from '../../schemas/categoryOfflineCourse.js';

export default class OfflineCourseModel
{
    constructor(){
        this.model = MongoDB.db.model('offline_course',OfflineCourseSchema);
        this.CategoryOfflineCourseModel = MongoDB.db.model('category_offline_course', CategoryOfflineCourseSchema);
        this.VideoOfflineCourseModel = MongoDB.db.model('video_offline_course', VideOfflineCourseSchema);
    }
    ...
    async getRowVideo(id){
        return await this.VideoOfflineCourseModel.findOne({"_id":id});
    }
    ...
    async #checkVideo(title,course_id){
        return await this.VideoOfflineCourseModel.findOne(
            {
    ...
    async addVideo(course_id,title,link,img,status){
        ...
        const row = new this.VideoOfflineCourseModel({
            course_id,
            title,
            link,
            img,
    ...
    async saveVideo(id,title,link,img,status){
        ...
        const result = await this.VideoOfflineCourseModel.updateOne({"_id":id},{
            "$set" : data 
        });    
        if(result?.modifiedCount > 0)
            return 1;
    ...
    async getVideoList(id){
         return await this.VideoOfflineCourseModel.find({"course_id":id});
    }
    ...
    async deleteImgVideoCourse(id)
    {
        ...
        await this.VideoOfflineCourseModel.updateOne({"_id":id},{
            "$set" : data
        });
    ...
    async changeStatusVideo(status_id,status_value){
        ...
        await this.VideoOfflineCourseModel.updateOne({"_id":status_id},{
            "$set" : data 
        });
    ...
    async deleteRowVideo(id){
        ...
        await this.VideoOfflineCourseModel.deleteOne({"_id":id});
        return 1;

    }

    async getCategories(){
        return await this.CategoryOfflineCourseModel.find().select({"_id":1,"title":1});
    }
