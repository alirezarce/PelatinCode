...
import CategoryBlog from '../../schemas/categoryBlog.js';
...
{
    constructor(){
        this.model = MongoDB.db.model('blog',BlogSchema);
        this.CategoryUserModel = MongoDB.db.model('user_group', UserGroupSchema);
        this.CategoryBlogModel = MongoDB.db.model('category_blog', CategoryBlog);


    }

    async add(title,category_id,status
        ,short_description,long_description,img,slug,title_seo
        ,description_seo){
    ...
    async getCategories(){
        return await this.CategoryBlogModel.find().select({"_id":1,"title":1});
    }
