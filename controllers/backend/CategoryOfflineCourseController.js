...
import CategoryOfflineCourseModel from '../../models/backend/categoryOfflineCourse.js';

export default class CategoryOfflineCourseController extends BackendController
{
    #url =  getEnv('BACKEND_URL') + 'categoryOfflineCourse/';

    constructor()
    {
        super();
        this.model = new CategoryOfflineCourseModel();
    }
    ...
    async add(req,res){
        try{
            ...
            const data = {
                "title" : 'ثبت دسته بندی دوره ی آفلاین  ',
                "form_data" : req?.session?.category_add_data,
                "mainCategory" : mainCategory,
                "logo" : logo
            }
            return res.render(this.templatePath+'categoryOfflineCourse/add',data);
        }
        catch(e){
    ...
    async postAdd(req,res){
        try{
            ...
            if(logo == '' && req?.files?.logo)    
            {
                ...
                if(ext !== '')
                {
                     const fileName = 'category_offline_course/' + fileNameGenerator('logo',ext);
    ...
    async index(req,res){
        try{
            ...
            const data = {
                "title" : 'لیست دسته بندی دوره های آفلاین',
            ...
            return res.render(this.templatePath+'categoryOfflineCourse/index',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
    ...
    async edit(req,res){
        try{
            ...
            const mainCategory = await this.model.getMainCategoryList();
            const data = {
                "title" : '  ویرایش دسته بندی  دوره های آفلاین',
                "back_url" : back_url,
                "mainCategory" : mainCategory,
                "row" : row,
                "delUrl" : currentUrl,
            }
            return res.render(this.templatePath+'categoryOfflineCourse/edit',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
