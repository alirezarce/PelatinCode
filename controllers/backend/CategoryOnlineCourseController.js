...
import CategoryOnlineCourseModel from '../../models/backend/categoryOnlineCourse.js';

export default class CategoryOnlineCourseController extends BackendController
{
    #url =  getEnv('BACKEND_URL') + 'categoryOnlineCourse/';

    constructor()
    {
        super();
        this.model = new CategoryOnlineCourseModel();
    }
    ...
    async add(req,res){
        try{
            ...
            const data = {
                "title" : 'ثبت دسته بندی دوره ی آنلاین  ',
                "form_data" : req?.session?.category_add_data,
                "mainCategory" : mainCategory,
                "logo" : logo
            }
            return res.render(this.templatePath+'categoryOnlineCourse/add',data);
        }
        catch(e){
    ...
    async postAdd(req,res){
        try{
            ...
            if(logo == '' && req?.files?.logo)    
            {
                ...
                if(ext !== '')
                {
                     const fileName = 'category_online_course/' + fileNameGenerator('logo',ext);
    ...
    async index(req,res){
        try{
            ...
            const data = {
                "title" : 'لیست دسته بندی دوره های آنلاین',
            ...
            return res.render(this.templatePath+'categoryOnlineCourse/index',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
    ...
    async edit(req,res){
        try{
            ...
            const mainCategory = await this.model.getMainCategoryList();
            const data = {
                "title" : '  ویرایش دسته بندی  دوره های آنلاین',
                "back_url" : back_url,
                "mainCategory" : mainCategory,
                "row" : row,
                "delUrl" : currentUrl,
            }
            return res.render(this.templatePath+'categoryOnlineCourse/edit',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
