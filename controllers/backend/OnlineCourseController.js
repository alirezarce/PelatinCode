...
import OnlineCourseModel from '../../models/backend/onlineCourse.js';
import axios from 'axios';


export default class OnlineCourseController extends BackendController
{
    #url =  getEnv('BACKEND_URL') + 'onlineCourse/';
    ...
    async add(req,res){
        try{
            ...
            const data = {
                "title" : 'ثبت  دوره آنلاین  ',
                "categories":categories,
                "form_data" : req?.session?.category_add_data,
                
            }
            return res.render(this.templatePath+'onlineCourse/add',data);
        }
        catch(e){
    ...
    async postAdd(req,res){
        try{
            ...
            req.session.onlineCourse_add_data = formData;
...
{                    
    if(req.files.img.size <= toByte(1000,'MB'))
    {
       ...
       if(ext !== '')
       {
            const fileName = 'img_onlineCourse/' + fileNameGenerator('img_onlineCourse',ext);
...
{                    
    if(req.files.img_video1.size <= toByte(1000,'MB'))
    {
       ...
       if(ext !== '')
       {
            const fileName = 'img_onlineCourse/' + fileNameGenerator('img_video1',ext);
            const path = getPath() + 'media/' + fileName;
...
{                    
    if(req.files.img_video2.size <= toByte(1000,'MB'))
    {
       ...
       if(ext !== '')
       {
            const fileName = 'img_onlineCourse/' + fileNameGenerator('img_video2',ext);
            const path = getPath() + 'media/' + fileName;
...
{                    
    if(req.files.img_video3.size <= toByte(1000,'MB'))
    {
       ...
       if(ext !== '')
       {
            const fileName = 'img_onlineCourse/' + fileNameGenerator('img_video3',ext);
            const path = getPath() + 'media/' + fileName;
                    ...
                    .post('https://www.skyroom.online/skyroom/api/apikey-31625588-6-052b7f494bab5ff1ffcb16c8b12bcee3',
                        {
                          ...
                          const room_id=res_api.data.result;

                    await this.model.saveSkyRoomId(resultAdd?._id,room_id);
    ...
    async index(req,res){
        try{
            ...
            return res.render(this.templatePath+'onlineCourse/index',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
    ...
    async edit(req,res){
        try{
            ...
            {
                const path = getPath() + 'media/img_onlineCourse' + row.img;
                await unlink(path);
            ...
            if(this.input(req.query.del) == '2')
                {
                    const path = getPath() + 'media/img_onlineCourse' + row.img_video1;
                    await unlink(path);
                ...
                if(this.input(req.query.del) == '3')
                    {
                        const path = getPath() + 'media/img_onlineCourse' + row.img_video2;
                        await unlink(path);
                    ...
                    if(this.input(req.query.del) == '4')
                        {
                            const path = getPath() + 'media/img_onlineCourse' + row.img_video3;
                            await unlink(path);
                        ...
                        if(this.input(req.query.del) == '5')
                            {
                                        const path = getPath() + 'media/img_onlineCourse' + row.medium_img;
                                        await unlink(path);
            ...
            const data = {
                "title" : 'ویرایش   دوره آنلاین',
                "back_url" : back_url,
                "row" : row,
                "categories":categories,
                "delUrl" : currentUrl,
            }
            return res.render(this.templatePath+'onlineCourse/edit',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
  ...
  {                    
    ...
    async addVideo(req,res){
        try{
            ...
            const data = {
                "title" :"اضافه کردن ویدیو  جدید برای دوره آنلاین " + course.title,
                "form_data" : req?.session?.video_add_data,
            }
            return res.render(this.templatePath + 'onlineCourse/add_video',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
...
{                    
    if(req.files.img.size <= toByte(1000,'MB'))
    {
       ...
       if(ext !== '')
       {
            const fileName = 'img_video_onlineCourse/' + fileNameGenerator('img',ext);
            const path = getPath() + 'media/' + fileName;
            await req.files.img.mv(path);
            img = fileName;
       }
    ...
    async listVideos(req, res){
        try{
            ...
            const data = {
                "title" :"لیست  ویدیو های دوره آنلاین " ,
                "rows":rows,
                "route_url":route_url,
                "delete_url" : delete_url,
                "url" : url,
            ...
            return res.render(this.templatePath + 'onlineCourse/list_videos',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
    ...
    async editVideo(req,res){
        try{
            ...
            if(this.input(req.query.del) == '1')
            {
                const path = getPath() + 'media/img_video_onlineCourse' + row.img;
                await unlink(path);
            ...
            return res.render(this.templatePath + 'onlineCourse/edit_video',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
