...
import CategoryFaceToFaceCourseModel from '../../models/backend/categoryFaceToFaceCourse.js';

export default class CategoryFaceToFaceCourseController extends BackendController
{
    #url =  getEnv('BACKEND_URL') + 'categoryFaceToFaceCourse/';

    constructor()
    {
        super();
        this.model = new CategoryFaceToFaceCourseModel();
    }
    ...
    async add(req,res){
        try{
            ...
            const data = {
                "title" : 'ثبت دسته بندی دوره حضوری جدید',
                "form_data" : req?.session?.category_add_data,
                "mainCategory" : mainCategory,
                "logo" : logo
            }
            return res.render(this.templatePath+'categoryFaceToFaceCourse/add',data);
        }
        catch(e){
    ...
    async postAdd(req,res){
        try{
            ...
            if(logo == '' && req?.files?.logo)    
            {
                ...
                if(ext !== '')
                {
                     const fileName = 'category_facetoface_course/' + fileNameGenerator('logo',ext);
    ...
    async index(req,res){
        try{
            ...
            const data = {
                "title" : 'لیست دسته بندی دوره حضوری ها',
            ...
            return res.render(this.templatePath+'categoryFaceToFaceCourse/index',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
    ...
    async edit(req,res){
        try{
            ...
            const mainCategory = await this.model.getMainCategoryList();
            const data = {
                "title" : 'ویرایش دسته بندی دوره حضوری',
                "back_url" : back_url,
                "mainCategory" : mainCategory,
                "row" : row,
                "delUrl" : currentUrl,
            }
            return res.render(this.templatePath+'categoryFaceToFaceCourse/edit',data);
        }
        catch(e){
            return super.toError(e,req,res);
        }
