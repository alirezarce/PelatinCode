$(document).ready(function() {
    ...
    $('#frm-rest-password').on('submit', function(e) {
        e.preventDefault();
        mobile = $('#mobile').val();
        ...
        $.post('/user/reset-password', { mobile: mobile ,password:password,confirm_password:confirm_password}, function(data) {
            if (data.success) {
              showOtpModal();
            } else {
                 Swal.fire({
                 ...
                 return new Promise((resolve) => {
                   ...
                   if (result.isConfirmed) {
                         Swal.fire({
                                icon: 'success',
                                title: 'تبریک!',
                                text: 'رمز شما با موفقیت تغییر یافت لطفا مجددا واردشوید  !',
                                confirmButtonText: 'تایید',
                            }).then(() => {
                                window.location.href = "user/dashboard"; // یا هر مسیری که میخواهید کاربر وارد شود
                           });
                    } else {
